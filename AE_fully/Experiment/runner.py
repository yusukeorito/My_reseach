...
class Runner:
    ...
    def preprocessing(self) -> np.ndarray:
        ...
        train_in = np.array([add_gaussian_noise(image, 0, 1, alpha=CFG.S) for image in X_train])
        train_out = X_train
        test_in = np.array([add_gaussian_noise(image, 0, 1, alpha=CFG.S) for image in X_test])
        test_out = X_test
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        ...
        model1 = create_fully_AEmodel(params=params,W=w_intializer1,B=bias_initializer1)
        history1 = model1.fit(X_train, y_train,
                ...
                callbacks=[LogEpochIntermediateCallcack(layer_name_list=self.layer_list,CFG=self.set,X_train_m=X_train_,
                                                        path=f"../Output/Spin/spinA_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed1']}.pkl")]
                )
        
        joblib.dump(history1.history, f"../Output/Loss/model001_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed1']}.pkl")
        ...
        model2 = create_fully_AEmodel(params=params,W=w_intializer2,B=bias_initializer2)
        ...
        history2 = model2.fit(X_train, y_train,
                ...
                validation_data=(X_test, y_test),
                callbacks=[LogEpochIntermediateCallcack(layer_name_list=self.layer_list,CFG=self.set,X_train_m=X_train_,
                                                        path=f"../Output/Spin/spinB_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed2']}.pkl")]
                )
        joblib.dump(history1.history, f"../Output/Loss/model002_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed2']}.pkl")
        
        
        spinA = joblib.load(f"../Output/Spin/spinA_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed1']}.pkl")
        spinB = joblib.load(f"../Output/Spin/spinB_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}_seed{CFG['seed2']}.pkl")
        
        print('save overlap')
        sim_q = get_sim_q(spinA,spinB,layer_name_list=self.layer_list,path=f"../Output/Overlap/time_sim_q_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}.pkl")
        qab, qaa, q2 = get_q2(spinA,spinB,layer_name_list=self.layer_list, path=f"../Output/Overlap/time_q2_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}.pkl")
        layer_q = get_layer_overlap(qab,qaa,q2,sim_q,layer_name_list=self.layer_list, path=f"../Output/Overlap/layer_q_{CFG['data_name']}_ini{CFG['ini_type']}_M{CFG['M']}_L{CFG['L']}_S{CFG['S']}.pkl")